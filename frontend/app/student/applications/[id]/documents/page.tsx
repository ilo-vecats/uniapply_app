        ...
        <div className="p-6">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h2 className="text-xl font-semibold text-slate-900 mb-2">Document Upload Wizard</h2>
                ...
                {requiredDocs.map((reqDoc: any) => {
                  ...
                  return (
                    <DocumentUploadCard
                      ...
                      status={status}
                      document={documents.find((d: any) => d.document_type === reqDoc.document_type)}
                      onDrop={(files) => onDrop(files, reqDoc.document_type)}
                      uploading={uploading}
                    />
                  )
                })}
              ...
              <div className="mt-6 flex justify-end gap-4">
                ...
                <button
                  onClick={async () => {
                    try {
                      await applicationsAPI.submit(applicationId as string)
...
function DocumentUploadCard({ documentType, isRequired, status, document, onDrop, uploading }: any) {
  ...
  return (
    <div className={`p-4 rounded-lg border-2 ${getStatusColor()}`}>
      ...
      {status === 'missing' && (
        <div {...getRootProps()} className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-500">
          <input {...getInputProps()} />
          {isDragActive ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {document.ai_extracted_data && 'AI data extracted'}
              </p>
            </div>
          </div>
          {status !== 'verified' && (
